"""
utils/command_debug.py — Registres statiques (cooldown, permission) +
orchestration du diagnostic complet d'une commande, extrait de
cogs/dev/debug_cmd.py (fichier devenu trop lourd.
...
⚠️ Valeurs à jour partiellement seulement (cf. remarque de l'auteur au
moment de l'extraction) — à corriger ultérieurement.
"""
...
def format_cooldown(command_name: str) -> str:
...
def format_permissions(command_name: str) -> str:
...
# ════════════════════════════════════════════════════════════
# 🔍 Orchestration — extrait de cogs/dev/debug_cmd.py
# ════════════════════════════════════════════════════════════

@dataclass
class CommandDebugInfo:
    """Résultat du diagnostic, prêt à être rendu par views/dev/debug_cmd_view.py."""
    command_name: str
    enabled: bool
    total: int
    today: int
...
async def get_command_debug_info(command_name: str) -> CommandDebugInfo | None:
    """Rassemble le diagnostic complet d'une commande.

    Renvoie None si la commande est inconnue (absente du système de
    maintenance, des registres statiques, et jamais utilisée) — évite de
    fournir un faux diagnostic pour un nom inventé.
    """
    ...
    return CommandDebugInfo(
        command_name=command_name,
        enabled=enabled,
        total=total,
        today=today,
        last_used_str=last_used_str,
        cooldown_str=format_cooldown(command_name),
        permission_str=format_permissions(command_name),
    )