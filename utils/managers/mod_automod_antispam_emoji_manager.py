"""
utils/managers/mod_automod_antispam_emoji_manager.py — CRUD Anti Spam Emoji.
"""
...
_DEFAULTS: dict = {"guild_id": None, "enabled": False, "max_emoji": 10}
_ALLOWED_FIELDS = {"enabled", "max_emoji"}
...
async def load_config(guild_id: int) -> dict:
    ...
    async with get_session() as session:
        row = await session.get(ModAutomodAntispamEmojiConfig, guild_id)
...
async def save_config(guild_id: int, **fields) -> dict:
    ...
    async with get_session() as session:
        row = await session.get(ModAutomodAntispamEmojiConfig, guild_id)
        if row is None:
            row = ModAutomodAntispamEmojiConfig(guild_id=guild_id, **clean)
            session.add(row)
        else:
