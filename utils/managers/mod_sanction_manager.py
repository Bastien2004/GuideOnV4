...
MAX_SOFTBAN_PURGE_SECONDS = 604_800
...
def _validate_tempban_duration(duration_seconds: int) -> int:
    ...
    if duration_seconds < MIN_TEMPBAN_SECONDS or duration_seconds > MAX_TEMPBAN_SECONDS:
        raise SanctionError(
            f"La durée du tempban doit être comprise entre **1h** et **365j**.",
            warning=True,
        )
    return duration_seconds
...
async def _persist_sanction(*, guild_id: int, user_id: int, moderator_id: int, type_: SanctionType, reason: str, duration_seconds: int | None = None, expires_at: datetime | None = None, dm_sent: bool = False) -> dict:
    """Ajoute une nouvelle sanction en base de donnée avec un ID unique."""
...
async def mute(guild_id: int, member: discord.Member, moderator_id: int, reason: str, duration_seconds: int, *, dm_sent: bool = False) -> dict:
    ...
    try:
        ...
        raise SanctionError("Le bot n'a pas la permission pour mute cet utilisateur.") from None

    except discord.HTTPException:
        log.exception("[MOD_SANCTION] Erreur du /mod mute guild=%s user=%s", guild_id, member.id)

        raise SanctionError("Une **erreur Discord** s'est produite lors du **mute**.") from None
...
async def unmute(member: discord.Member, moderator_id: int, reason: str | None = None) -> dict:
    ...
    if active is None:
        raise SanctionError("Ce membre n'a actuellement aucun **mute actif**.", warning=True)

    try:
        await member.timeout(None, reason=reason or "Retrait anticipé du mute")

    except discord.Forbidden:
        raise SanctionError("Le bot n'a pas la permission de **unmute** ce membre.") from None

    except discord.HTTPException:
        log.exception("[MOD_SANCTION] Échec du /mod unmute guild=%s user=%s", member.guild.id, member.id)
        raise SanctionError("Une **erreur Discord** s'est produite lors du **unmute**.") from None
...
async def kick(guild_id: int, member: discord.Member, moderator_id: int, reason: str, *, dm_sent: bool = False) -> dict:
    ...
    try:
        ...
        log.exception("[MOD_SANCTION] Échec du /mod kick guild=%s user=%s", guild_id, member.id)
        raise SanctionError("Une **erreur Discord** s'est produite lors de l'**expulsion**.") from None
...
async def ban(guild: discord.Guild, target: discord.abc.Snowflake, moderator_id: int, reason: str, *, dm_sent: bool = False, delete_message_seconds: int = 0) -> dict:
    """Application d'un bannissement permanent."""
