...
softban, conformément à la convention utils/ (logique métier + mutations
Discord). Les cogs délèguent ici, puis mappent SanctionError vers
error_container/warning_container.

Historique 100% consultatif (le "casier judiciaire") : aucune escalade
automatique, confirmé avec Paul — get_user_history/get_user_stats ne font
que lire, jamais déclencher d'action.

Mute : timeout natif Discord (member.timeout), pas de rôle custom — borné
par la limite dure de Discord (28 jours). Ban/tempban : gérés via
guild.ban/guild.unban ; l'expiration d'un tempban est traitée par
cogs/events/mod_tempban_scheduler.py (boucle périodique), PAS ici.
"""
...
# ============================================================
# 🔒 Bornes de sécurité (jamais dépassables, même par un admin)
# ============================================================

MIN_REASON_LENGTH = 3
MAX_REASON_LENGTH = 500

MIN_MUTE_SECONDS = 60             # 1 minute
MAX_MUTE_SECONDS = 2_419_200      # 28 jours — limite DURE de Discord (member.timeout)

MIN_TEMPBAN_SECONDS = 3_600       # 1 heure
MAX_TEMPBAN_SECONDS = 31_536_000  # 365 jours

MIN_SOFTBAN_PURGE_SECONDS = 0
MAX_SOFTBAN_PURGE_SECONDS = 604_800  # 7 jours — limite DURE de l'API Discord (delete_message_seconds)


# Libellés d'affichage (emoji, label) par type — réutilisés par les vues
# (DM de notification, historique) pour éviter toute duplication de mapping.
...
class SanctionError(Exception):
    ...
    warning=True : refus attendu (ex. "ce membre n'a aucun mute actif"),
    à afficher via warning_container. warning=False : échec réel (ex. le
    bot n'a pas la permission), à afficher via error_container.
    """
...
def _as_aware(dt: datetime) -> datetime:
    """
    Traite un datetime naïf comme UTC avant comparaison.

    SQLite (utilisé en dev/fallback, cf. bot.py) ne persiste pas toujours
    l'info de fuseau horaire des colonnes DateTime(timezone=True) — les
    valeurs relues peuvent revenir "naïves". PostgreSQL (prod) n'a pas ce
    problème, mais on normalise dans tous les cas par sécurité.
    """
...
def _validate_mute_duration(duration_seconds: int) -> int:
    if duration_seconds < MIN_MUTE_SECONDS or duration_seconds > MAX_MUTE_SECONDS:
        raise SanctionError(
            f"La durée du mute doit être entre **{MIN_MUTE_SECONDS}s** et **{MAX_MUTE_SECONDS}s** "
            "(28 jours max — limite imposée par Discord).",
            warning=True,
        )
...
def _validate_tempban_duration(duration_seconds: int) -> int:
    if duration_seconds < MIN_TEMPBAN_SECONDS or duration_seconds > MAX_TEMPBAN_SECONDS:
        raise SanctionError(
            f"La durée du tempban doit être entre **{MIN_TEMPBAN_SECONDS}s** (1h) et "
            f"**{MAX_TEMPBAN_SECONDS}s** (365 jours).",
            warning=True,
        )
    return duration_seconds
    ...
    *,
    ...
    async with get_session() as session:
        ...
        row = Sanction(
            ...
            active=(type_ in (SanctionType.MUTE, SanctionType.BAN, SanctionType.TEMPBAN)),
            dm_sent=dm_sent,
        )
        session.add(row)
        await session.flush()
...
# ============================================================
# ⚖️ Sanctions — mutation Discord + persistance
# ============================================================

async def warn(guild_id: int, user_id: int, moderator_id: int, reason: str, *, dm_sent: bool = False) -> dict:
    """Avertissement — aucune mutation Discord, juste un enregistrement."""
...
async def unban(guild: discord.Guild, user_id: int, moderator_id: int, reason: str | None = None) -> dict:
    """Lève un ban (permanent ou temporaire) avant son expiration naturelle."""
    ...
    try:
        ...
        # Déjà débanni manuellement côté Discord, hors du bot — on nettoie quand même la DB.
...
async def softban(
    ...
    *, dm_sent: bool = False, purge_seconds: int | None = None,
    ...
    """Softban : ban immédiatement suivi d'un unban, pour purger l'historique de messages."""
    reason = _validate_reason(reason)
    ...
    try:
        await guild.ban(member, reason=f"[SOFTBAN] {reason}", delete_message_seconds=purge_seconds)
        await guild.unban(member, reason="Softban — levée immédiate après purge des messages")
    except discord.Forbidden:
...
async def get_active_ban_sanction(guild_id: int, user_id: int) -> dict | None:
    """Ban actif (permanent ou temporaire non-expiré) d'un membre, None sinon."""
    async with get_session() as session:
        rows = (
            await session.execute(
                select(Sanction).where(
                    Sanction.guild_id == guild_id,
                    Sanction.user_id == user_id,
                    Sanction.type.in_((SanctionType.BAN, SanctionType.TEMPBAN)),
                    Sanction.active.is_(True),
                ).order_by(Sanction.created_at.desc())
            )
        ).scalars().all()
