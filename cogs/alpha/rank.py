...
def _build_rank_announcement(membre: discord.Member, grade: str, is_promotion: bool, old_grade: str | None, journaliste_only: bool = False) -> LayoutView:
    """Construction de l'annonce du rank-up."""
    ...
    if journaliste_only and old_grade and old_grade != "journaliste":
        c.add_item(TextDisplay(f"<:Alpha:1500414179650048070> <@{membre.id}> rejoint l'équipe des **Journalistes** ! 📰"))
        ...
        c.add_item(TextDisplay(
            f"<:Alpha:1500414179650048070> Bienvenue à <@{membre.id}> qui rejoint l'équipe des **{label}** !"))
...
def _build_journaliste_message(pseudo_jeu: str, grade: str, journaliste_ping_id: int | None, is_promotion: bool) -> LayoutView:
    """Construction du message pour les journalistes."""
...
def _build_dev_message(pseudo_jeu: str, dev_ping_id: int | None) -> LayoutView:
    """Construction du message pour les développeurs (emoji head)."""
...
@app_commands.describe(membre="Membre Discord à rank-up", pseudo_jeu="Pseudo NationsGlory du membre", grade="Grade à attribuer (journaliste = statut cumulable)")
...
async def rank(interaction: Interaction, membre: discord.Member, pseudo_jeu: str, grade: app_commands.Choice[str]) -> None:
    ...
    # 🗞️ BRANCHE A — Attribution du statut Journaliste
