"""
Commande /ng mmr — Affiche le MMR d'un pays sur un serveur NationsGlory.
"""
...
async def get_cached_sheet() -> pd.DataFrame:
    ...
    async with _cache_lock:
        ...
        try:
            ...
            log.exception("Erreur lors de la récupération des données de MMR")
...
def clean_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    required_columns = ["pays", "serveur", "mmr"]
...
def filter_server_dataframe(df: pd.DataFrame, serveur: str) -> pd.DataFrame:
    ...
    df_srv = df[(df["serveur"] == serveur) & df["mmr"].notna()]

    if df_srv.empty:
        raise ValueError(f"Aucune donnée trouvée pour `{serveur}`.")

    return df_srv.groupby("pays", as_index=False)["mmr"].sum()
...
def compute_mmr_stats(df_srv: pd.DataFrame, pays: str) -> dict:
    """Calcule les statistiques MMR du pays."""
    df_srv = df_srv.sort_values(by="mmr", ascending=False).reset_index(drop=True)
...
def build_top3_text(df: pd.DataFrame) -> str:
    ...
    return "\n".join(
        f"**#{i + 1}** {row['pays'].title()} — `{int(row['mmr'])}`"
...
def build_mmr_view(pays: str, serveur: str, stats: dict) -> tuple[LayoutView, discord.File | None]:
    """Construit la view principale."""
    view      = LayoutView(timeout=None)
    container = Container()

    container.add_item(TextDisplay(f"# ⚔️ MMR — {pays.title()} ({serveur.capitalize()})"))
    container.add_item(Separator())

    container.add_item(TextDisplay(
        "## 📊 Statistiques du pays\n\n"
        f"- **MMR :** `{stats['mmr_value']}`\n"
...
async def get_mmr_data(serveur: str, pays: str) -> dict:
    ...
    return compute_mmr_stats(df_srv, pays)
...
@app_commands.command(name="mmr", description="⚔️ Affiche le MMR d'un pays")
@app_commands.describe(pays="Nom du pays")
@app_commands.choices(serveur=SERVER_CHOICES)
async def mmr(interaction: Interaction, serveur: str, pays: str):
    ...
    if not await verifier_commande(interaction, "ng_mmr"):
        return

    # 📊 Tracking
    await tracker_commande(interaction, "ng_mmr")

    # 📦 Chargement et traitement des données
    try:
        serveur = serveur.lower().strip()
        pays    = pays.lower().strip()
        stats   = await get_mmr_data(serveur, pays)

        view, file = build_mmr_view(pays, serveur, stats)
        ...
        log.exception("Erreur commande /ng mmr")
        await interaction.followup.send(
...
@mmr.error
async def mmr_error(interaction: Interaction, error: app_commands.AppCommandError):
