"""
cogs/dev/kick.py — /dev kick

Fait quitter GuideOn d'un serveur Discord (par ID).
Action irréversible sans réinvitation → vue de confirmation obligatoire.
...
class _ConfirmKickView(LayoutView):
    ...
    def _build(self) -> None:
        ...
        c = Container()
        c.add_item(TextDisplay("# ⚠️ Confirmation — Quitter un serveur"))
        c.add_item(Separator())
        c.add_item(TextDisplay(
            f"GuideOn va **quitter définitivement** ce serveur :\n\n"
            f"**Nom :** {g.name}\n"
            f"**ID :** `{g.id}`\n"
            f"**Propriétaire :** {owner}\n"
            f"**Membres :** `{g.member_count}`\n\n"
            f"⚠️ Le bot ne pourra **plus revenir** sans nouvelle invitation. Confirmer ?"
        ))
        c.add_item(Separator())

        btn_confirm = Button(label="✅ Confirmer", style=ButtonStyle.danger, custom_id="kick_confirm")
        btn_cancel  = Button(label="↩️ Annuler",  style=ButtonStyle.secondary, custom_id="kick_cancel")
    ...
    async def _on_confirm(self, interaction: Interaction) -> None:
        ...
        try:
            ...
            await interaction.edit_original_response(
                view=error_container("Une erreur Discord est survenue lors du départ.")
            )
            return
...
@app_commands.command(name="kick", description="🚪 [DEV] Fait quitter GuideOn d'un serveur")
@app_commands.describe(id_serveur="ID du serveur à quitter")
async def kick(interaction: Interaction, id_serveur: str) -> None:

    if not await check_dev(interaction, "faire quitter le bot d'un serveur"):
        return
