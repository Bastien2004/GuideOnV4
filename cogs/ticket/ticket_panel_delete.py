...
async def ticket_panel_delete(interaction: discord.Interaction, lien_panel: str) -> None:
    if not await verifier_ban_utilisateur(interaction):
        return
    if not await check_admin(interaction, "supprimer un **panel de tickets**"):
        return
    ...
    confirm = ConfirmView(
        ...
        question=(
            ...
            "-# Le message sera retiré et les tickets liés effacés de la base."
        ),
    ...
    if not confirm.confirmed:
        return  # annulé ou timeout : ConfirmView a déjà mis à jour le message
    ...
    # 📂 Suppression DB (cascade tickets + staff roles)
...
async def ticket_panel_delete_error(
