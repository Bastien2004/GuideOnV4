...
"""
cogs/ticket/ticket_ban.py — /ticket ban

Bannit un utilisateur des tickets en lui attribuant le rôle de ban configuré
sur le panel (staff uniquement). Protège le staff et les admins.
"""
...
async def ticket_ban(interaction: discord.Interaction, utilisateur: discord.Member) -> None:
    ...
    if not await is_staff(interaction, ticket, guild_id):
        return await interaction.followup.send(
            view=error_container("Vous n'avez pas la permission de bannir des tickets."),
            ephemeral=True,
        )

    await tracker_commande(interaction, "ticket_ban")

    # 🛡️ Protection : on ne bannit pas le staff ni les admins
    ...
    if is_target_staff or utilisateur.guild_permissions.administrator:
        return await interaction.followup.send(
            view=error_container("Impossible de bannir un membre du staff."), ephemeral=True
        )
    ...
    if ban_role in utilisateur.roles:
        return await interaction.followup.send(
            view=error_container("Cet utilisateur est déjà banni des tickets."),
            ephemeral=True,
        )
    ...
    try:
        ...
        await interaction.followup.send(
            view=error_container("Je n'ai pas les permissions pour ajouter ce rôle."),
            ephemeral=True,
        )
