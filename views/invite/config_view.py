...
from utils.managers.invite_manager import load_invite_config, reset_invite_config, save_invite_config
...
async def create_invite_view(guild_id: int, bot, author_id: Optional[int] = None) -> Optional[LayoutView]:
    """Construit l'interface /invite config."""
...
def _cb_pick_role(guild_id, bot, author_id):
    ...
    async def cb(interaction: Interaction):
        ...
        async def on_select(sel: Interaction, role_ids: list[int]):
            ...
            await sel.response.edit_message(
                view=success_container(f"Rôle-récompense défini sur {role.mention} !")
            )
            new_view = await create_invite_view(guild_id, bot, author_id)
            if new_view:
                try:
                    ...
                    log.warning("[Invite] Mise à jour de la view après sélection rôle impossible")
        ...
        c = Container()
        c.add_item(TextDisplay("🎁 Choisis le **rôle-récompense** :"))
