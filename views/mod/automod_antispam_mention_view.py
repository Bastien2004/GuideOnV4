"""
views/mod/automod_antispam_mention_view.py — Interface de configuration du système Anti Spam Mention.
"""
...
async def create_automod_antispam_mention_view(guild_id: int, bot, author_id: Optional[int] = None) -> Optional[LayoutView]:
    """Construction de la view de configuration Anti Spam Mention."""
    ...
    c.add_item(TextDisplay(f"# <:protect_config:1539608365704028340> Système anti spam mention"))
    ...
    c.add_item(Section(
        TextDisplay(
            "**🔘 Statut du système**\n"
            "-# Protège l'abus de mention par message."
        ),
    ...
    c.add_item(Section(
        TextDisplay(
            ...
            f"-# Bloque les messages avec plus de `{max_mentions}` mentions."
        ),
        accessory=btn_max,
    ))
    c.add_item(Separator())
