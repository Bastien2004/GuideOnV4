"""
views/alpha/config_nota_view.py — Dashboard de configuration du système de notations Alpha.
...
from utils.managers.alpha_nota_manager import load_nota_config, save_nota_config
...
class NotaConfigView(LayoutView):
    ...
    async def _reload(self, i: Interaction) -> None:
        ...
        await i.response.edit_message(view=NotaConfigView(self.guild_id, cfg, self.owner_id))

    async def _on_salons(self, i: Interaction) -> None:
        cfg = await load_nota_config(self.guild_id)
        await i.response.edit_message(view=_SalonsView(self.guild_id, cfg, self.owner_id))

    async def _on_config(self, i: Interaction) -> None:
        cfg = await load_nota_config(self.guild_id)
        await i.response.edit_message(view=_RoleCfgView(self.guild_id, cfg, self.owner_id))
...
def _back_to_main(guild_id, owner_id):
    async def _fn(i: Interaction):
        cfg = await load_nota_config(guild_id)
        await i.response.edit_message(view=NotaConfigView(guild_id, cfg, owner_id))
    return _fn
...
class _SalonsView(LayoutView):
    ...
    def _build(self) -> None:
        ...
        btn.callback = _back_to_main(self.guild_id, self.owner_id)
    ...
    def _make_save(self, field: str):
        async def _fn(i: Interaction, ch: int) -> None:
            self.cfg = await save_nota_config(self.guild_id, **{field: ch})
            await i.response.edit_message(view=_SalonsView(self.guild_id, self.cfg, self.owner_id))
        return _fn
...
class _RoleCfgView(LayoutView):
    ...
    def _build(self) -> None:
        ...
        btn_back.callback = _back_to_main(self.guild_id, self.owner_id)
        c.add_item(ActionRow(btn_pays, btn_url, btn_back))
        c.add_item(TextDisplay("-# GuideOn Studio"))
        self.add_item(c)

    async def _save(self, i: Interaction, field: str, value) -> None:
        self.cfg = await save_nota_config(self.guild_id, **{field: value})
        await i.response.edit_message(view=_RoleCfgView(self.guild_id, self.cfg, self.owner_id))
    ...
    async def _on_pays(self, i: Interaction) -> None:
        async def on_submit(inter: Interaction, val: str) -> None:
            ...
            self.cfg = await save_nota_config(self.guild_id, countries_count=n)
            await inter.response.edit_message(view=_RoleCfgView(self.guild_id, self.cfg, self.owner_id))
        modal = TextModal(
    ...
    async def _on_url(self, i: Interaction) -> None:
        async def on_submit(inter: Interaction, val: str) -> None:
            self.cfg = await save_nota_config(self.guild_id, url_country_lookup=val.strip() or None)
            await inter.response.edit_message(view=_RoleCfgView(self.guild_id, self.cfg, self.owner_id))
        modal = TextModal(
...
class _HorairesView(LayoutView):
    ...
    def _build(self) -> None:
        ...
        btn_bck.callback = _back_to_main(self.guild_id, self.owner_id)
    ...
    async def _open_time_modal(self, i: Interaction, which: str) -> None:
        ...
        async def on_submit(inter: Interaction, val: str) -> None:
            ...
            self.cfg = await save_nota_config(self.guild_id, **{wd_f: wd_val, h_f: hh, m_f: mm})
            await inter.response.edit_message(view=_HorairesView(self.guild_id, self.cfg, self.owner_id))
...
class _ParamsView(LayoutView):
    ...
    async def _on_toggle(self, i: Interaction) -> None:
        self.cfg = await save_nota_config(self.guild_id, enabled=not self.cfg.get("enabled", True))
